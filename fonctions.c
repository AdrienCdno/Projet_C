...
int Bannissable(int k, int nlig, int ncol, T_Tab_Case *T, struct T_Case _case)
{
    ...
    if(_case.y == 1 && _case.x == ncol) //coin en bas à gauche
        bannissable=1;
    else if(_case.y == nlig && _case.x == 1) //coin en haut à droite
        bannissable=1;
    ...
    for(m = 0; m < k; m++)
    {
        if (_case.y==(*T).tab[m].y+1 && _case.x==(*T).tab[m].x-1) //diagonale haut droite
            ...
            diagonal=1;
        else if (_case.y==(*T).tab[m].y && _case.x==(*T).tab[m].x) //case bannie sur elle-même
            diagonal=1;
    }
...
void Calcul_Nimbers(int nim[][VMAX], int nlig, int ncol)
{
    ...
    for(x=ncol-1;x>=0;x--)//commercer par le dernier case,vertical
    {
        ...
        for(y=nlig-1;y>=0;y--)//commercer par le dernier case,honrizontal
        {
            nim[x][y]=nimber;
            nimber++;
            nimber%=3;//modulo pour cotroler inferieur à 3
        }
        nimber_tempal+=2;
        nimber_tempal%=3;
    }
    ...
    for(i=(_case.x)+1; i<=(_case.x)+2; i++) //boucle qui teste pour les 2 cases en dessous de la case du pion
        {
            ...
            if((bannie == 0)&&(i <= nlig))
            {
                nb_vois++; //incrémentation de nb_vois
                (*vois).tab[nb_vois-1].x=i; //stockage de la coordonnée x de la case voisine
                (*vois).tab[nb_vois-1].y=_case.y; //stockage de la coordonnée y de la case voisine
            }
        }
    bannie=0;
    for(k=(_case.y)+1; k<=(_case.y)+2; k++)//boucle qui teste pour les 2 cases à droite de la case du pion
        {
            if(nban!=0) //s'il y a des cases bannies
            {
                for(j = 0; j <= nban; j++)
            ...
            {
                nb_vois++; //incrémentation de nb_vois
                (*vois).tab[nb_vois-1].x=_case.x; //stockage de la coordonnée x de la case voisine
                (*vois).tab[nb_vois-1].y=k; //stockage de la coordonnée y de la case voisine
            }
        }
}
